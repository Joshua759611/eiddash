    ...
    public function __getDateData($request, &$dateString, &$excelColumns, &$title, &$briefTitle)
    {
    	...
    	if ($request->testtype == 'VL') {
            ...
            if ($request->indicatortype == 1 || $request->indicatortype == 6) {
                ...
                $selectStr .= ",$table.regimen_name as infantprophylaxis, $table.receivedstatus_name as receivedstatus, $table.labcomment, $table.reason_for_repeat, $table.spots, $table.feeding_name, entry_points.name as entrypoint, ir.name as infantresult, $table.mother_prophylaxis_name as motherprophylaxis, mr.name as motherresult, $table.mother_age, $table.mother_ccc_no, $table.mother_last_result, case  when $table.order_no IS NOT NULL then 'EMR' when $table.site_entry  = '1, 1, NULL' then 'Site_entry' when $table.site_entry = '2, 1, 0' then 'POC_sites' else 'Lab' end ";
