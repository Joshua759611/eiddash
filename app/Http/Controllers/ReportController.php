    ...
    public function __getDateData($request, &$dateString, &$excelColumns, &$title, &$briefTitle)
    {
    	...
    	if ($request->testtype == 'VL') {
            ...
            if ($request->indicatortype == 2) {
                $excelColumns = ['System ID', 'Batch','Patient CCC No', 'Lab Tested In', 'County', 'Sub-County', 'Partner', 'Facilty', 'Facility Code', 'Gender', 'DOB', 'Age', 'Sample Type', 'Date Collected', 'Justification', 'Date Received', 'Date Tested', 'Date Dispatched', 'ART Initiation Date', 'Received Status', 'Reasons for Repeat', 'Rejected Reason', 'Regimen', 'Regimen Line', 'PMTCT', 'Result','Entry'];
                $selectStr .= ", $table.receivedstatus_name as receivedstatus, $table.reason_for_repeat, viralrejectedreasons.name as rejectedreason, $table.prophylaxis_name as regimen, viralregimenline.name as regimenline, viralpmtcttype.name as pmtct, $table.result, case  when $table.order_no IS NOT NULL then 'EMR' when $table.site_entry  = '1, 1, NULL' then 'Health Facility' when $table.site_entry = '2, 1, 0' then 'POC' else 'Testing Lab' end ";
