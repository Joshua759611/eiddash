    ...
    public function __getDateData($request, &$dateString, &$excelColumns, &$title, &$briefTitle)
    {
    	...
    	if ($request->testtype == 'VL') {
            ...
            if ($request->indicatortype == 1 || $request->indicatortype == 6) {
                $excelColumns = ['System ID','Sample ID', 'Batch', 'Lab Tested In', 'County', 'Sub-County', 'Partner', 'Facilty', 'Facility Code', 'Gender', 'DOB', 'Age (Months)', 'PCR Type', 'Enrollment CCC No', 'Date Collected', 'Date Received', 'Date Tested', 'Date Dispatched', 'Infant Prophylaxis', 'Received Status', 'Lab Comment', 'Reason for Repeat', 'Spots', 'Feeding', 'Entry Point', 'Result', 'PMTCT Intervention', 'Mother Result', 'Mother Age', 'Mother CCC No', 'Mother Last VL', 'Sample Entry Location'];
                $selectStr .= ",$table.regimen_name as infantprophylaxis, $table.receivedstatus_name as receivedstatus, $table.labcomment, $table.reason_for_repeat, $table.spots, $table.feeding_name, entry_points.name as entrypoint, ir.name as infantresult, $table.mother_prophylaxis_name as motherprophylaxis, mr.name as motherresult, $table.mother_age, $table.mother_ccc_no, $table.mother_last_result, case  when $table.order_no IS NOT NULL then 'EMR' when $table.site_entry  = '1, 1, NULL' then 'Site_entry' when $table.order_no = 'NULL, 0, 1' then 'EMR' when $table.site_entry = '2, 1, 0' then 'POC_sites' else 'Lab' end ";
