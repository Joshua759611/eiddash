...
class ConsumptionsController extends Controller
{
	...
	public function create(BlankRequest $request) {
		...
		foreach ($consumptions as $key => $consumption) {
			...
			$existing = Consumption::existing($consumption->year, $consumption->month, $consumption->lab_id)->get();
			
			if ($existing->isEmpty()) {
				// New consumption to be saved
				$consumption_model = new Consumption();
				$saveconsumption = $consumption_model->saveConsumption($consumption);
			} else {
				$saveconsumption = $existing->first();
			}
			return response()->json([
	            'status' => 'ok',
	            'consumptions' => $saveconsumption,
	        ], 201);
